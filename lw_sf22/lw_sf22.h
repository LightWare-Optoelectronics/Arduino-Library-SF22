...
class LW_SF22{
	public:
		LW_SF22(HardwareSerial &serial1);
		LW_SF22(HardwareSerial &serial1, HardwareSerial &print);//, TwoWire &wire);
		LW_SF22(uint8_t Wire_i2c, HardwareSerial &print);
		...
		void writeSerialBaudrate(uint32_t Rate);

		// This will change the I2C interface Address
		void writeI2CAddress(uint32_t Address);

		// This will change the Sampling Rate
		void writeSamplingRate(uint16_t Rate);
		...
		// This will change the Sampling Rate
		void i2cWriteSamplingRate(uint16_t Rate);
